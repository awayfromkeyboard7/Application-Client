...
export default function Voice({ team }) {
  ...
  const muteVoice = () => {
    memberVoiceRefs.current[1].pause();
  };
  
  return (
    <div className={styles.voiceBox}>
      <div className={voiceOff ? styles.voiceBtnDisable : styles.voiceBtn} onClick={voiceOff ? voiceChat : muteVoice}>{voiceOff ? `팀 보이스 ON` : `팀 보이스 OFF`}</div>
    {
      team?.map((member, idx) => 
        <div className={voiceOff ? styles.voiceIconDisable : styles.voiceIcon} key={idx}>
